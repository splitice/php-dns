...
class DNSQuery
{
    ...
    function ReadRecord()
    {
        ...
        switch ($typeid) {
            ...
            default : // something we can't deal with
                $this->ReadResponse($ans_header ['length']);
                break;
        }
    ...
    function SmartALookup($hostname, $depth = 0)
    {
        ...
        if ($best_answer_typeid == "A")
            return $best_answer; // got an IP ok
        ...
        for ($a = 0; $a < $this->lastadditional->count; $a++) {
            if (($this->lastadditional->results [$a]->domain == $hostname) && ($this->lastadditional->results [$a]->typeid == "A"))
